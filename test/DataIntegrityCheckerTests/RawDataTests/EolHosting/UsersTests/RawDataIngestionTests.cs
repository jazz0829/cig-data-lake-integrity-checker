...
namespace DataIntegrityCheckerTests.RawDataTests.EolHosting.UsersTests
{
	...
	{
		private string sqlTableName = "raw.HOST_CIG_Users";
		private string athenaTableName = "\"customerintelligence_raw\".\"object_host_cig_users\"";
		...
		public void data_in_source_and_athena_tables_match()
		{
			...
			var sqlData = sqlRepo.GetTopFromTable(sqlTableName, "CAST(ID AS varchar(36)),CIGCopyTime,CAST(GoogleClientID AS varchar(36)) DESC,UPPER(Environment)", limit);
			var actual = awsRepo.GetTopFromTable(athenaTableName, "id,cigcopytime,googleclientid,UPPER(environment)", limit);

			actual.Count.Should().BeGreaterThan(0);

			var columnNames = (sqlData.First() as IDictionary<string, object>).Keys;
			var transformation = new RawUsersTransformation();
