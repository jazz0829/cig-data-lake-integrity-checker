...
namespace DataIntegrityCheckerTests.RawDataTests.EolHosting.CreditManagementStatusTests
{
	[UseReporter(typeof(DiffReporter))]
	public class RawDataIngestionTests : IDisposable
	{
		private string sqlTableName = "raw.HOST_CIG_CreditManagementStatus";
		private string athenaTableName = "\"customerintelligence_raw\".\"object_host_cig_creditmanagementstatus\"";
		...
		public void data_in_source_and_athena_tables_match()
		{
			...
			var sqlData = sqlRepo.GetTopFromTable(sqlTableName, "CAST(ID AS varchar(36)),CIGCopyTime", limit);
			var actual = awsRepo.GetTopFromTable(athenaTableName, "id,cigcopytime", limit);

			actual.Count.Should().BeGreaterThan(0);

			var columnNames = (sqlData.First() as IDictionary<string, object>).Keys;
			var transformation = new RawCreditManagementStatusTestsTransformation();
			foreach (var columnName in columnNames)
			{
				//if (columnName == "ID" || columnName == "Code" || columnName == "Description") continue;
				var sqlColumnValues = "";
