...
namespace DataIntegrityCheckerTests.RawDataTests.EolHosting.ContractEventsTests
{
	[UseReporter(typeof(DiffReporter))]
	public class RawDataIngestionTests : IDisposable
	{
		private string sqlTableName = "raw.HOST_CIG_ContractEvents";
		private string athenaTableName = "\"customerintelligence_raw\".\"object_host_cig_contractevents\"";
		...
		public void data_in_source_and_athena_tables_match()
		{
			...
			var sqlData = sqlRepo.GetTopFromTable(sqlTableName, "CAST(ID AS varchar(36)),CIGCopyTime,UPPER(Environment)", limit);
			var actual = awsRepo.GetTopFromTable(athenaTableName, "id,cigcopytime,UPPER(environment)", limit);

			actual.Count.Should().BeGreaterThan(0);

			var columnNames = (sqlData.First() as IDictionary<string, object>).Keys;
			var transformation = new RawContractEventsTransformation();
