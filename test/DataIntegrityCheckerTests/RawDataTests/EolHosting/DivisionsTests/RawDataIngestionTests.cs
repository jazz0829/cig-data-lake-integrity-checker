...
namespace DataIntegrityCheckerTests.RawDataTests.EolHosting.DivisionsTests
{
	...
	{
		private string sqlTableName = "raw.HOST_CIG_Divisions";
		private string athenaTableName = "\"customerintelligence_raw\".\"object_host_cig_divisions\"";
		...
		public void data_in_source_and_athena_tables_match()
		{
			...
			var sqlData = sqlRepo.GetTopFromTable(sqlTableName, "CAST(Code AS varchar(36)),CIGCopyTime,UPPER(Environment)", limit);
			var actual = awsRepo.GetTopFromTable(athenaTableName, "code,cigcopytime,UPPER(environment)", limit);

			actual.Count.Should().BeGreaterThan(0);

			var columnNames = (sqlData.First() as IDictionary<string, object>).Keys;
			var transformation = new RawDivisionsTransformation();
