...
namespace DataIntegrityCheckerTests.RawDataTests.EolHosting.ContractMutationsTests
{
	...
	{
		private string sqlTableName = "raw.HOST_CIG_ContractMutations";
		private string athenaTableName = "\"customerintelligence_raw\".\"object_host_cig_contractmutations\"";
		...
		public void data_in_source_and_athena_tables_match()
		{
			...
			var sqlData = sqlRepo.GetTopFromTable(sqlTableName, "CAST(ID AS varchar(36)),CAST(HID AS varchar(36)) ASC,CIGCopyTime,UPPER(Environment)", limit);
			var actual = awsRepo.GetTopFromTable(athenaTableName, "id,hid,cigcopytime,UPPER(environment)", limit);

			actual.Count.Should().BeGreaterThan(0);

			var columnNames = (sqlData.First() as IDictionary<string, object>).Keys;
			var transformation = new RawContractMutationsTransformation();
			foreach (var columnName in columnNames)
			{
				// TODO: the mismatch is skipped because ContractMutations not used yet
				if (columnName == "HID")
				{
					continue;
				}
